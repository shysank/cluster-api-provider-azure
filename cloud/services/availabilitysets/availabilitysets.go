...
func (s *Service) Reconcile(ctx context.Context) error {
	...
	maxFaultDomainCountStr, ok := asSku.GetCapability(resourceskus.MaximumPlatformFaultDomainCount)
	if !ok {
		return errors.Errorf("cannot find capability %s sku %s", resourceskus.MaximumPlatformFaultDomainCount, *asSku.Name)
	}

	maxFaultDomainCount, err := strconv.ParseUint(maxFaultDomainCountStr, 10, 32)
	if err != nil {
	...
	for _, asSpec := range s.Scope.AvailabilitySetSpecs() {
		...
		faultDomainCount := asSpec.DesiredFaultDomainCount
		if asSpec.DesiredFaultDomainCount > int32(maxFaultDomainCount) {
			s.Scope.V(2).Info("desired fault domain count is greater than max fault domain count, will use the max fault domain count",
				"desired fault domain count", asSpec.DesiredFaultDomainCount, "max fault domain count", maxFaultDomainCount)
			faultDomainCount = int32(maxFaultDomainCount)
		}
		...
		asParams := compute.AvailabilitySet{
			...
			AvailabilitySetProperties: &compute.AvailabilitySetProperties{
				PlatformFaultDomainCount: pointer.Int32Ptr(faultDomainCount),
			},
...
func (s *Service) Delete(ctx context.Context) error {
	...
	for _, asSpec := range s.Scope.AvailabilitySetSpecs() {
		...
		if err != nil {
			return errors.Wrapf(err, "failed to delete availability set %s", asSpec.Name)
		}
